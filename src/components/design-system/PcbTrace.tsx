...
export function CircuitBackground() {
  ...
  useEffect(() => {
    ...
    const edges: { from: number; to: number; progress: number; speed: number; particles: { t: number; active: boolean }[] }[] = [];
    ...
    const rebuild = () => {
      ...
      // Connect horizontally and vertically (sparse)
      for (let r = 0; r < ROWS; r++) {
    ...
    const draw = () => {
      ...
      // Draw animated particles along traces
      for (const e of edges) {
        for (const p of e.particles) {
          if (!p.active) { p.t += e.speed * 0.3; if (p.t > 1.2) { p.t = -0.2; p.active = Math.random() > 0.3; } continue; }
          p.t += e.speed;
          ...
          // Manhattan path: first segment horizontal, second vertical
